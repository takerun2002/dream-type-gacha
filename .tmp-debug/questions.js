...
exports.analyzePointDistribution = analyzePointDistribution;
/**
 * 質問設計のバランス目標:
 * - 選択式8問 × 4選択肢 = 32選択肢
 * - 各選択肢: メイン(+2) × 1, サブ(+1) × 1
...
function calculateResult(answers) {
    ...
    answers.forEach((answer) => {
        if (answer.answerId) {
            const question = exports.questions.find((q) => q.id === answer.questionId);
    ...
    if (topTypes.length > 1) {
        ...
        const sortedTopTypes = topTypes
            .slice()
            .sort((a, b) => { var _a, _b; return ((_a = rank[a]) !== null && _a !== void 0 ? _a : 999) - ((_b = rank[b]) !== null && _b !== void 0 ? _b : 999); });
        const seed = answers
            .filter((a) => a.answerId)
        ...
        (() => {
            try {
                if (typeof window === "undefined") {
                    fetch("http://127.0.0.1:7243/ingest/5be1a6a7-7ee8-4fe8-9b00-19e37afd0e10", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                            sessionId: "debug-session",
                            runId: "type-glitch-post-fix",
                            hypothesisId: "H-A",
                            location: "src/lib/questions.ts:calculateResult(tie_break)",
                            message: "tie detected -> deterministic tie-break applied",
                            data: { topTypes: sortedTopTypes, maxScore, chosen },
                            timestamp: Date.now(),
                        }),
                    }).catch(() => { });
                }
